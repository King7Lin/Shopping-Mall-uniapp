	...
	export default {
		data() {
		 ...
		 onLoad(option) {
			...
			// 详情
			uni.request({
				url:`http://1.12.230.158:3007/all/selectDetails?shop_id=${option.shop_id}&user_id=${this.user_id}`,
				// url:`http://1.12.230.158:3007/all/selectDetails?shop_id=3&user_id=1`,
				success: (res) => {
			...
			// 地址
			uni.request({
				url:'http://1.12.230.158:3007/all/selectAddress?user_id=' + this.user_id,
				success: (res) => {
					console.log(res)
			...
			uni.request({
				url:'http://1.12.230.158:3007/all/insertfootprint?user_id=' + this.user_id +'&shop_id=' + option.shop_id,
				success: (res) => {
					console.log(res)
				}
			}) 
		...
		methods: {
			...
			buttonClick(e) {
				...
				if(this.desc ==='' || a !== this.specifications.length){
					...
					if(e.index == 0){
						uni.request({
							url:'http://1.12.230.158:3007/all/insertCart',
							method:'POST',
							header:{
						...
						console.log(shop1)
						uni.request({
							url:'http://1.12.230.158:3007/all/insertOrder',
							method:'POST',
							header:{
							'content-type':'application/x-www-form-urlencoded'
							},
			...
			ChangeCheck(){
				...
				if(this.checkList){
					uni.request({
						url:'http://1.12.230.158:3007/all/insertCollection?user_id='+this.user_id+'&shop_id='+this.shop.shop_id,
						success: (res) => {
							console.log(res)
							if(res.data.status !== 1){
					...
					uni.request({
						url:'http://1.12.230.158:3007/all/deleteCollection?user_id='+this.user_id+'&shop_id='+this.shop.shop_id,
						success: (res) => {
							console.log(res)
							if(res.data.status !== 1){
